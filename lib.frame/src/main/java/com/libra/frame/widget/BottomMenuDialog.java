...
public class BottomMenuDialog extends BottomSheetDialogFragment implements View.OnClickListener {
    ...
    private void initRootView() {
        ...
        for (int i = 0; i < titles.size(); i++) {
            ...
            if (i == 0) {
                ...
                childView.setBackground(getDrawableListByType(true, true, false, false));
                continue;
            }
            if (separate) {
                if (i == (titles.size() - 2)) {
                    childView = initView(titles.get(i), i + 1, false, true);
                    childView.setBackground(getDrawableListByType(false, false, true, true));
                    continue;
                }
                if (i == (titles.size() - 1)) {
                    //false, false, true, true
                    childView = initView(titles.get(i), i + 1, false, false);
                    childView.setBackground(getDrawableListByType(true, true, true, true));
                    continue;
                }
            } else {
                if (i == (titles.size() - 1)) {
            ...
            childView.setBackground(getDrawableListByType(false, false, false, false));
        }
    ...
    public void setLastItemSeparate(boolean separate) {
